  ...
  return (
    <BleedSection className="relative py-20 md:py-24">
      <div className="max-w-4xl">
        <h2 className="font-syne text-3xl leading-[1.05] tracking-[-0.02em] text-foreground md:text-5xl">
      ...
      <div className="mt-10 grid gap-6 lg:grid-cols-2">
        {HOMEPAGE_PROOF_PATTERNS.map((pattern) => {
          ...
          return (
            <SectionSurface key={pattern.projectType} className="overflow-hidden">
              {visual ? (
                <CloudinaryProofImage
                  ...
                  className="h-52 w-full border-b border-foreground/10 object-cover md:h-60"
                />
              ) : (
                <>
                  {/* TODO: Add approved homepage visual mapping for this pattern publicId. */}
                  <div className="flex h-52 w-full items-center justify-center border-b border-foreground/10 bg-surface text-xs uppercase tracking-[0.18em] text-foreground/45 md:h-60">
                    Visual Pending
                  </div>
                </>
              )}
              ...
              <div className="p-6 md:p-7">
                <h3 className="font-syne text-2xl leading-tight text-foreground">{pattern.headline}</h3>
                <p className="mt-2 text-sm text-signal">{pattern.plainEnglishLabel}</p>
                ...
                <dl className="mt-6 space-y-4">
                  ...
                  <div>
                    ...
                    <dd className="mt-1 text-sm leading-relaxed text-foreground/84">{pattern.impact}</dd>
                  </div>
                </dl>

                <Link
                  href={pattern.ctaHref}
                  className="group mt-6 inline-flex items-center gap-2 text-sm text-foreground"
                >
