...
function ProofCard({ study }: { study: CaseStudy }) {
  ...
  return (
    ...
    >
      <SectionSurface className="relative isolate h-full min-h-72 transition-[transform,border-color,box-shadow] duration-300 group-hover:-translate-y-1 group-hover:border-signal/30 group-hover:shadow-[0_30px_80px_rgba(15,217,200,0.16)]">
        ...
        <div className="relative z-10 flex h-full flex-col p-6">
          ...
          <p className="mt-5 font-syne text-[clamp(2rem,4vw,3.4rem)] leading-[0.95] tracking-[-0.02em] text-foreground">
            {study.primaryMetric.value}
          </p>
          <p className="mt-1 font-mono text-[0.66rem] uppercase tracking-[0.16em] text-foreground/52">
          ...
          <div className="mt-6 border-t border-foreground/10 pt-4">
            ...
            <p className="mt-1.5 line-clamp-3 text-sm leading-relaxed text-body-muted">
              {study.outcomeHeadline}
            </p>
          </div>

          <div className="mt-auto flex items-end justify-between pt-6">
            <span className="max-w-[60%] font-mono text-[0.62rem] uppercase tracking-[0.16em] text-foreground/40">
              {study.clientContext}
            </span>
    ...
    <BleedSection className="relative py-20 md:py-28">
      <div className="mb-12 grid items-end gap-6 md:grid-cols-[minmax(0,1fr)_auto] md:gap-10">
        <div className="max-w-3xl">
          <p className="font-mono text-xs uppercase tracking-[0.22em] text-signal/80">Proof</p>
        ...
        <Link
          href="/proof"
          className="group inline-flex items-center gap-2 self-start rounded-full border border-foreground/16 bg-foreground/4 px-4 py-2 font-mono text-[0.7rem] uppercase tracking-[0.18em] text-foreground/82 transition-colors hover:border-brand/40 hover:text-foreground"
        >
          All proof
